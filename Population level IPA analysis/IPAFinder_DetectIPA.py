...
parser.add_argument("-b",dest = 'bamfile',action = "store",type = str,help = "Input certain RNA-seq bamfile")
                        ...
                        if upstream_cov > 0.5 and downstream_cov < 0.2:
                            ...
                            for key,value in skipstart_dict.items():
                                ...
                                if len([feature for feature in annot[label] if feature[1] == exon_rank]) == 1:
                                    ...
                                    if IPUI < 1:
                                        IPA_inf = label + ";" + feature + "_" + str(rank) + ";" + strand + ";" +  IPAtype + ";" + IPA_region + ";" + exon_region + ";" + str(IPUI)
...
outfile.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format("Genename","Intron","Strand","IPAtype","IPA_terminal_region","Upstream_exon_region","IPUI","Samplename"))
input_tuple = list(zip(annot.keys(),[args.bamfile]*len(annot)))
...
for gene_list in result_list:
    if gene_list != []:
        for IPA in gene_list:
            if IPA != []:
                outfile.write("{}\t{}\n".format('\t'.join(IPA.split(";")),args.bamfile))
