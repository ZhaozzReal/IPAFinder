...
parser.add_argument("-N",dest='Normal_bamfile',action="store",type=str,help="Normal bamfile")
parser.add_argument("-T",dest="Tumor_bamfile",action="store",type=str,help="Tumor bamfile")
...
parser.add_argument("-output",dest="outfile",action="store",type=str,help="Output all Intron PAsite and IPARatio")
...
all_bamfiles=[args.Normal_bamfile,args.Tumor_bamfile]
cigar_char = ('M', '=', 'X')
minaqual=10
...
def GetIPAratio(input_tuple):
    ...
    for feature,rank,chrom,start,end,strand,length,exon_rank_left,exon_rank_right in annot[label]:
        if feature=="intron" and int(rank)==intronrank:
            ...
            if len(exon_iv) ==1:
                ...
                if sum(np.array(exon_abundance)>10)==2 and sum(np.array([x-y for x,y in zip(exon_abundance,IPA_isoform_abundance)])>0)==2:
    ...
    return [IPAevent]+IPARatio_list+[P_val]
...
first_line=['Label','Normal','Tumor','Pval']
out.writelines("\t".join(first_line)+"\n")
for lst in result_list:
    if float(lst[1])!=1 and float(lst[2])!=1:
