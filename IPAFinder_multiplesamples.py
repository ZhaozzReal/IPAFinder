...
parser = ArgumentParser(description="De novo analysis of Intronic Polyadenylation from standard RNA-seq (IpA analysis of multiple samples between different conditions)")
        ...
        if feature == "intron" and int(rank) == intronrank:
            ...
            if len(exon_iv) == 1:
                ...
                if sum(np.array(exon_abundance)>10) == len(all_bamfiles) and sum(np.array([x-y for x,y in zip(exon_abundance,IPA_isoform_abundance)])>0)>len(all_bamfiles)*0.5:
                    ...
                    IPA_condition1 = np.mean(np.array(IPA_isoform_abundance[:num_condition1]))
                    IPA_condition2 = np.mean(np.array(IPA_isoform_abundance[num_condition1:]))
                    FL_condition1 = np.mean(np.array(exon_abundance[:num_condition1]))-np.mean(np.array(IPA_isoform_abundance[:num_condition1]))
                    FL_condition2 = np.mean(np.array(exon_abundance[num_condition1:]))-np.mean(np.array(IPA_isoform_abundance[num_condition1:]))
                    if FL_condition1 > 0 and FL_condition2 > 0:
                        ratio_val,P_val = sp.stats.fisher_exact([[IPA_condition1,FL_condition1],[IPA_condition2,FL_condition2]])
                        result.append([IPAevent]+IPARatio_list+[IPUI_condition_diff,P_val])
    return result
...
for lst in result_list:
    if lst != []:
        out.writelines('\t'.join(list(map(str,lst[0])))+'\n')
